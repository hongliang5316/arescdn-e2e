...
- 目录刷新相关的检查可能偶发失败：basic 组的"目录刷新后返回 v2"，share 组的 4 项"拿到 v2"。两层节点各自拉取配置，生效时间最多差约 15 秒。边缘层先用上新的目录版本号时，会从还在用旧版本号的回源层拿到旧内容，并按新版本号缓存下来，这次刷新就失效了。这是产品的问题，不是测试的问题，修复方案见实验环境文档"层间配置不同步"。2026-09-26 在同一份代码上跑了 3 轮，其中 2 轮出现。
